...
class AudioRuleDialog(wx.Dialog):
    ...
    def __init__(
      ...
      # Speech Behavior Radio Box
        speechBehaviorChoices = [_("Do not speak anything"), _("Keep original text"), _("Edit spoken text")]
        self.speechBehaviorRadioBox = sHelper.addItem(wx.RadioBox(self, label=_("Speech Action"), choices=speechBehaviorChoices))
        self.speechBehaviorRadioBox.Bind(wx.EVT_RADIOBOX, self.onSpeechBehavior)
        self.typeControls[audioRuleWave].append(self.speechBehaviorRadioBox)
        self.typeControls[audioRuleBuiltInWave].append(self.speechBehaviorRadioBox)
        self.typeControls[audioRuleBeep].append(self.speechBehaviorRadioBox)
        
      # Custom Speech Text
        self.customSpeechTextCtrl = sHelper.addLabeledControl(_("Custom Text"), wx.TextCtrl)
        self.typeControls[audioRuleWave].append(self.customSpeechTextCtrl)
        self.typeControls[audioRuleBuiltInWave].append(self.customSpeechTextCtrl)
        self.typeControls[audioRuleBeep].append(self.customSpeechTextCtrl)
      ...
      # Translators: label for custom label
        customLabelText = _("Custom Display Name (optional)")
        self.customLabelTextCtrl=sHelper.addLabeledControl(customLabelText, wx.TextCtrl)
    ...
    def editRule(self, rule):
        ...
        self.customLabelTextCtrl.SetValue(getattr(rule, "customLabel", ""))
    ...
    def makeRule(self):
        ...
        try:
            result = AudioRule(
                ...
                speechBehavior=self.speechBehaviorRadioBox.GetSelection(),
                customSpeechText=self.customSpeechTextCtrl.GetValue(),
                customLabel=self.customLabelTextCtrl.GetValue(),
            )
            return result
        except Exception as e:
            log.error(f"Could not add Audio Rule: {e}")
    ...
    def onSpeechBehavior(self, evt):
        if self.speechBehaviorRadioBox.GetSelection() == 2:
            self.customSpeechTextCtrl.Enable()
        else:
            self.customSpeechTextCtrl.Disable()
...
class RulesDialog(SettingsPanel):
    ...
    title = _("Audio Themes: Earcons & Speech Rules")
    ...
    def makeSettings(self, settingsSizer):
      ...
      # Search box for filtering rules
        self.ruleSearchEdit = sHelper.addLabeledControl(_("&Search rules:"), wx.TextCtrl)
        self.ruleSearchEdit.Bind(wx.EVT_TEXT, self.onRuleSearchChanged)

      # Statistics label
        self.statsLabel = wx.StaticText(self, -1, "")
        sHelper.addItem(self.statsLabel)
      ...
      # Buttons
        ...
        self.duplicateButton = bHelper.addButton(self, label=_("D&uplicate"))
        self.duplicateButton.Bind(wx.EVT_BUTTON, self.OnDuplicateClick)
        self.removeButton = bHelper.addButton(self, label=_("Re&move rule"))
        self.removeButton.Bind(wx.EVT_BUTTON, self.OnRemoveClick)
        self.testRuleButton = bHelper.addButton(self, label=_("T&est rule"))
        self.testRuleButton.Bind(wx.EVT_BUTTON, self.OnTestRuleClick)
      # Batch buttons
        bHelper2 = sHelper.addItem(guiHelper.ButtonHelper(orientation=wx.HORIZONTAL))
        self.enableAllButton = bHelper2.addButton(self, label=_("Enable A&ll"))
    ...
    def prepareRulesForFrenzy(self, frenzyType):
        ...
        self._filterDisplayedRules()

    def _filterDisplayedRules(self):
        query = getattr(self, '_searchQuery', '').lower()
        if query:
            self.displayedRules = [r for r in self.frenzyRules if query in r.getDisplayName().lower()]
            if hasattr(self, 'moveUpButton'):
                self.moveUpButton.Disable()
                self.moveDownButton.Disable()
        else:
            self.displayedRules = list(self.frenzyRules)
            if hasattr(self, 'moveUpButton'):
                self.moveUpButton.Enable()
                self.moveDownButton.Enable()

    def _refreshList(self):
        self.rulesList.SetItemCount(len(self.displayedRules))
        self.rulesList.Refresh()
        self._updateStats()

    def _updateStats(self):
        if not hasattr(self, 'statsLabel'):
            return
        total = len(self.displayedRules)
        enabled = sum(1 for r in self.displayedRules if r.enabled)
        disabled = total - enabled
        self.statsLabel.SetLabel(
            _("Total: {total} | Enabled: {enabled} | Disabled: {disabled}").format(
                total=total, enabled=enabled, disabled=disabled
            )
        )

    def onRuleSearchChanged(self, evt):
        self._searchQuery = self.ruleSearchEdit.GetValue()
        self._filterDisplayedRules()
        self._refreshList()

    def onEnableAllClick(self, evt):
        for rule in self.displayedRules:
            rule.enabled = True
        self._refreshList()
        core.callLater(100, lambda: ui.message(_("All rules enabled")))

    def onDisableAllClick(self, evt):
        for rule in self.displayedRules:
            rule.enabled = False
        self._refreshList()
    ...
    def onToggleClick(self,evt):
        if self.rulesList.GetSelectedItemCount()!=1:
            return
        index=self.rulesList.GetFirstSelected()
        self.displayedRules[index].enabled = not self.displayedRules[index].enabled
        self.rulesList.RefreshItem(index)
        if self.displayedRules[index].enabled:
            msg = _("Rule enabled")
        else:
            msg = _("Rule disabled")
    ...
    def OnAddClick(self,evt):
        ...
        if entryDialog.ShowModal()==wx.ID_OK:
            ...
            self._filterDisplayedRules()
            self._refreshList()
            index = len(self.displayedRules) - 1
            if index >= 0:
    ...
    def OnEditClick(self,evt):
        ...
        if editIndex<0:
            return
        disallowedFrenzyValues = [rule.getFrenzyValue() for rule in self.frenzyRules]
        allowedFrenzyValue = self.displayedRules[editIndex].getFrenzyValue()
        ...
        entryDialog.editRule(self.displayedRules[editIndex])
        if entryDialog.ShowModal()==wx.ID_OK:
            old_rule = self.displayedRules[editIndex]
            new_rule = entryDialog.rule
            self.displayedRules[editIndex] = new_rule
            try:
                idx = self.frenzyRules.index(old_rule)
                self.frenzyRules[idx] = new_rule
            except ValueError:
                pass
            self.rulesList.RefreshItem(editIndex)
            self.rulesList.SetFocus()
        entryDialog.Destroy()

    def OnDuplicateClick(self, evt):
        if self.rulesList.GetSelectedItemCount()!=1:
            return
        editIndex=self.rulesList.GetFirstSelected()
        if editIndex<0:
        ...
        # Create a deep copy of the selected rule
        import copy
        new_rule = copy.copy(self.displayedRules[editIndex])
        
        # Append " - Copy" to the custom label or comment if applicable
        if hasattr(new_rule, "customLabel") and new_rule.customLabel:
            new_rule.customLabel += _(" - Copy")
        elif hasattr(new_rule, "comment") and new_rule.comment:
            new_rule.comment += _(" - Copy")
            
        entryDialog.editRule(new_rule)
    ...
    def OnMoveClick(self,evt, increment):
        ...
        if 0 <= newIndex < len(self.displayedRules):
            # Swap
            tmp = self.displayedRules[index]
            self.displayedRules[index] = self.displayedRules[newIndex]
            self.displayedRules[newIndex] = tmp
            try:
                idx1 = self.frenzyRules.index(tmp)
                idx2 = self.frenzyRules.index(self.displayedRules[index])
                self.frenzyRules[idx1], self.frenzyRules[idx2] = self.frenzyRules[idx2], self.frenzyRules[idx1]
            except ValueError:
                pass
            self.rulesList.RefreshItem(index)
            self.rulesList.RefreshItem(newIndex)
    ...
    def OnRemoveClick(self,evt):
        index=self.rulesList.GetFirstSelected()
        if index >= 0:
            old_rule = self.displayedRules[index]
            del self.displayedRules[index]
            try:
                self.frenzyRules.remove(old_rule)
            except ValueError:
                pass
            self.rulesList.SetItemCount(len(self.displayedRules))
            self.rulesList.Refresh()
            if index < len(self.displayedRules):
                self.rulesList.Select(index)
            elif len(self.displayedRules) > 0:
                self.rulesList.Select(index - 1)
        self.rulesList.SetFocus()

    def OnTestRuleClick(self, evt):
        if self.rulesList.GetSelectedItemCount() != 1:
            return
        index = self.rulesList.GetFirstSelected()
        if index < 0:
            return
        rule = self.displayedRules[index]
        if not hasattr(self, '_lastTestTime'):
            self._lastTestTime = 0
        now = time.time()
        repeat = (now - self._lastTestTime) < 1
        self._lastTestTime = now
