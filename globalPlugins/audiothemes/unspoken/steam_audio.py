...
class SteamAudio:
	def __init__(self, dll_path=None):
		...
		if dll_path:
			...
			fallback_arch = "x86" if arch == "x64" else "x64"
			lib_name = "steam_audio.dll"
			
			# New paths
			paths_to_try.append(os.path.join(addon_dir, "lib", arch, lib_name))
			paths_to_try.append(os.path.join(addon_dir, "lib", fallback_arch, lib_name))
			# old path
			paths_to_try.append(os.path.join(addon_dir, lib_name))

		for path in paths_to_try:
			if not os.path.exists(path):
				log.debug(f"Steam Audio DLL not found at: {path}")
				continue
			try:
				self.dll = ctypes.CDLL(path)
				self._setup_function_signatures()
				log.debug(f"Steam Audio DLL loaded from: {path}")
				break # success
			except Exception as e:
				log.warning(f"Failed to load Steam Audio DLL from {path}: {e}")
				self.dll = None

		if not self.dll:
			raise FileNotFoundError(f"Steam Audio DLL not found in any of the attempted paths: {paths_to_try}")
	...
	def process_sound(self, input_buffer, angle_x, angle_y):
		...
		# Convert to ctypes array via array.array for ~100x faster bulk copy
	...
	def apply_reverb(self, input_buffer):
		...
		num_samples = len(input_buffer) // 2
		input_array = (ctypes.c_int16 * num_samples).from_buffer_copy(input_buffer)
